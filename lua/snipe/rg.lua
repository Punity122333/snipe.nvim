...
local function full_picker()
    ...
    -- initial query may be empty, so start with empty results
    render_results()
end
...
local function buffer_picker(file)
    ...
    vim.api.nvim_set_hl(0, "RgBufSep",      { fg = "#ff9e64" })   -- orange separator
    vim.api.nvim_set_hl(0, "RgBufLnum",     { fg = "#73daca" })

    -- ── Single buffer + window (one box) ──────────────────────────────────
    local buf = vim.api.nvim_create_buf(false, true)
    ...
    -- ── Anonymous namespaces (no collision across calls) ──────────────────
    ...
    local function get_icon_data(fp)
        if not has_devicons then return "", "RgBufNormal" end
    ...
    -- ── render(): writes ALL lines atomically, then applies highlights ─────
    -- Buffer layout (0-indexed):
    --   line 0  →  prompt  " >  <query>"
    --   line 1  →  sep     " ──────────"
    --   line 2+ →  results (one per entry)
    local function render()
        ...
        -- Write atomically; rendering flag suppresses the on_lines callback
        rendering = true
        vim.bo[buf].modifiable = true
        vim.api.nvim_buf_set_lines(buf, 0, -1, false, disp)
        rendering = false
        -- Leave modifiable=true so the user can type on line 1

        -- Clear all highlight namespaces, then re-paint
        for _, ns in ipairs({ ns_prompt, ns_sep, ns_sel, ns_cursor, ns_lnum, ns_icon }) do
            vim.api.nvim_buf_clear_namespace(buf, ns, 0, -1)
        end

        -- Prompt arrow (cyan ">" on line 0)
    ...
    -- ── navigation ────────────────────────────────────────────────────────
    local function next_res()
        if selected < #results then selected = selected + 1; render() end
    end
    local function prev_res()
    ...
    -- ── cursor guard: keep cursor on line 1, after the prompt prefix ──────
    vim.api.nvim_create_autocmd({ "CursorMoved", "CursorMovedI" }, {
        buffer = buf,
        callback = function()
            ...
            if r > 1 then
                -- Snap back to end of current query on line 1
    ...
    -- ── on_lines: react only to changes on line 1 (the prompt) ───────────
    vim.api.nvim_buf_attach(buf, false, {
        on_lines = function(_, _, _, firstline)
            -- Skip events we ourselves fired during render()
            if rendering then return end
            -- Only care about the prompt line (0-indexed line 0)
            if firstline ~= 0 then return end
            local raw = vim.api.nvim_buf_get_lines(buf, 0, 1, false)[1] or ""
            -- Recover query even if user managed to corrupt the prompt prefix
            if raw:sub(1, #PROMPT) ~= PROMPT then
                query = vim.trim(raw:gsub("^%s*>?%s*", ""))
            else
                query = raw:sub(#PROMPT + 1)
            end
            -- Debounce: cancel any in-flight timer and reschedule
