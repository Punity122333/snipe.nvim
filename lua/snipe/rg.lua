...
local rg_hl_done = false
local rg_buf_hl_done = false
local rg_hl_au_done = false

local function ensure_rg_hl_autocmd()
    if rg_hl_au_done then
        return
    end
    rg_hl_au_done = true
    vim.api.nvim_create_autocmd("ColorScheme", {
        callback = function()
            rg_hl_done = false
            rg_buf_hl_done = false
        end,
    })
end

local function setup_rg_hl()
    ensure_rg_hl_autocmd()
    if rg_hl_done then
        return
    end
    rg_hl_done = true
...
local function setup_rg_buf_hl()
    ensure_rg_hl_autocmd()
    if rg_buf_hl_done then
        return
    end
    rg_buf_hl_done = true
