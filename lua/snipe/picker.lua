...
--   build_items   fn(cb) (async; builds full list once for non-live pickers)
...
function M.open_picker(opts)
    ...
    local function update_view_start()
        if #filtered == 0 then
            view_start = 1
            return
        end
        if #filtered <= results_h then
            view_start = 1
            return
        end
        local max_start = math.max(1, #filtered - results_h + 1)
        if view_start > max_start then
            view_start = max_start
        end
        if selected < view_start then
            view_start = selected
        elseif selected > view_start + results_h - 1 then
            view_start = selected - results_h + 1
        end
    end
    ...
    local function highlight_selected()
        ...
        if #filtered > 0 and filtered[selected] then
            local row = selected - view_start + 1
            local max_row = math.min(results_h, #filtered - view_start + 1)
            if row >= 1 and row <= max_row then
                vim.api.nvim_buf_set_extmark(results_buf, ns_sel, row - 1, 0, {
                ...
                vim.api.nvim_buf_set_extmark(results_buf, ns_cur, row - 1, 0, {
                    virt_text = { { " > ", "NavCursor" } },
                    virt_text_pos = "overlay",
                    hl_mode = "combine",
                })
                pcall(vim.api.nvim_win_set_cursor, results_win, { row, 0 })
            end
        end
    ...
    local function render_results()
        ...
        if #filtered == 0 then
            ...
            update_view_start()
            local view_end = math.min(#filtered, view_start + results_h - 1)
            local di = 1
            for i = view_start, view_end do
                ...
                display[di] = text
                row_hls[di] = rdata.highlights or {}
                row_match_hls[di] = rdata.match_hl
                di = di + 1
            end
        end
    ...
    if opts.build_items then
        set_ro(results_buf, { "   (loading…)" })
        vim.schedule(function()
            opts.build_items(function(items)
                all_items = items or {}
                filtered = opts.filter_items and opts.filter_items(all_items, query) or all_items
                selected = 1
                vim.schedule(render_results)
    ...
    elseif opts.live and opts.get_items then
        set_ro(results_buf, { "   (loading…)" })
        vim.schedule(function()
            opts.get_items(query, function(items)
                filtered = items
