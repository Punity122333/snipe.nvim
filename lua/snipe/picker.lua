...
function M.open_picker(opts)
  ...
  local backdrop_win = vim.api.nvim_open_win(backdrop_buf, false, {
    relative = "editor", width = W, height = H, row = 0, col = 0,
    style = "minimal", border = "none", zindex = 40, focusable = false,
  })
  ...
  local preview_buf, preview_win =
    make_win(right_w - 3, box_h - 1, box_row, box_col + left_w + 2, "Preview", 50)
  ...
  local function update_preview()
    ...
    if not pdata then set_ro(preview_buf, { " (no preview)" }); return end

    local raw_lines = pdata.lines or {}
    local padded = {}
    for i, v in ipairs(raw_lines) do padded[i] = " " .. v:gsub("\t", "    ") end
    set_ro(preview_buf, padded)

    if pdata.syntax and pdata.syntax ~= "" then
      pcall(function() vim.bo[preview_buf].syntax = pdata.syntax end)
    end
    ...
    if focus and focus >= 1 and focus <= #padded then
      ...
      if hq and hq ~= "" then
        ...
        local cur_leftcol = tonumber(
          vim.fn.win_execute(preview_win, "echo winsaveview()['leftcol']"):match("%d+")) or 0
        ...
        vim.fn.win_execute(preview_win, string.format(
          "call winrestview({'lnum':%d,'col':%d,'leftcol':%d,'topline':%d})",
          focus, match_col, leftcol, topline))
        ...
        if padded[focus] then
          ...
          if #match_str > 0 then
            vim.api.nvim_buf_set_extmark(preview_buf, ns_prev_cur, focus - 1, match_col, {
              end_col = match_end + 1, hl_group = "SrchMatchCur", priority = 200,
            })
          end
        end
      else
        local topline = math.max(1, focus - math.floor(win_h / 2))
        vim.fn.win_execute(preview_win, string.format(
          "call winrestview({'lnum':%d,'topline':%d})", focus, topline))
  ...
  local function highlight_selected()
    ...
    if #filtered > 0 and filtered[selected] then
      vim.api.nvim_buf_set_extmark(results_buf, ns_sel, selected - 1, 0, {
        hl_group = "NavSelected", hl_eol = true,
        end_row = selected, end_col = 0, priority = 100,
      })
  ...
  local function render_results()
    ...
    if #filtered == 0 then display = { "   (no results)" } end
  ...
  local function open_selected()
    if #filtered == 0 or not filtered[selected] then return end
    local item = filtered[selected]
    close()
    vim.schedule(function()
      if opts.open_item then opts.open_item(item, origin_win) end
    end)
  end

  local function next_res()
    if selected < #filtered then selected = selected + 1; highlight_selected() end
  end
  local function prev_res()
    if selected > 1 then selected = selected - 1; highlight_selected() end
  end
