...
local function files_picker()
  ...
  local function build_cmd(query)
    if use_fd then
      local cmd = { fd_bin, "--type", "f", "--color=never", "--hidden", "--exclude", ".git", "--exclude", "node_modules" }
  ...
  for _, bufnr in ipairs(vim.api.nvim_list_bufs()) do
    if vim.bo[bufnr].buflisted then
      ...
      if name ~= "" then
        ...
        items[#items + 1] = { bufnr = bufnr, name = name, modified = vim.bo[bufnr].modified, lnum = info and info.lnum or 1 }
      end
    end
  end
  open_picker({
    title = "Buffers", all_items = items,
...
local function marks_picker()
  ...
  for c in ("abcdefghijklmnopqrstuvwxyz"):gmatch(".") do
    ...
    if lnum ~= 0 and cur_file ~= "" then
      ...
      items[#items + 1] = { mark = c, file = cur_file, lnum = lnum, col = pos[3], text = lines[1] and vim.trim(lines[1]) or "" }
    end
  end
  ...
  open_picker({
    title = "Marks", all_items = items,
    filter_items = function(all, q) return filter(all, q, function(it) return it.mark .. " " .. it.file .. " " .. it.text end) end,
    render_item = function(item, _)
...
local function references_picker()
  ...
  vim.lsp.buf_request_all(0, "textDocument/references", params, function(responses)
    ...
    vim.schedule(function()
      if #items == 0 then vim.notify("No references found for: " .. word, vim.log.levels.WARN); return end
      open_picker({
        title = "Refs: " .. word, all_items = items,
        filter_items = function(all, q) return filter(all, q, function(it) return it.file .. " " .. it.text end) end,
        render_item  = function(item, max_len) return render_file_row(item.file, tostring(item.lnum), item.text:sub(1, 50), max_len) end,
        preview_item = function(item) return file_preview(item.file, item.lnum) end,
...
local function oldfiles_picker()
  ...
  open_picker({
    title = "Recent Files", all_items = items,
...
local function projects_picker()
  ...
  for _, sf in ipairs(vim.fn.glob(session_dir .. "/*.vim", false, true) or {}) do
    ...
    if vim.fn.isdirectory(path) == 1 and not seen[path] then seen[path] = true; items[#items + 1] = path end
  end
  for _, f in ipairs(vim.v.oldfiles or {}) do
    local dir = vim.fn.fnamemodify(f, ":h")
    if dir ~= "" and vim.fn.isdirectory(dir) == 1 and not seen[dir] then seen[dir] = true; items[#items + 1] = dir end
  end
  open_picker({
    title = "Projects", all_items = items,
    filter_items = function(all, q) return filter(all, q) end,
    render_item = function(item, _)
